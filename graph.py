...
def isCompatiblePos(n):
	...
	for i in nodes:
		if i[3]==True:
			...
			for j in range(problem.getNbOpponents()):
				if math.sqrt((problem.getOpponent(j)[0]-nodes[n][1])**2 + (problem.getOpponent(j)[1]-nodes[n][2])**2) < problem.robot_radius*2:
					return False
	return True
...
def maxDegreeHeuristic(G, Sol):
	...
	while(dominatingSet(G) == False):
		...
		print("[%f, %f]," % (nodes[nodeDegreeMax][1], nodes[nodeDegreeMax][2]))
		nbDefense += 1
	return nbDefense
